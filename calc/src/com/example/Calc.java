...
public class Calc {
	...
	public static void main(String[] args) {
		...
		for (; !("+".equals(operation)) && !("-".equals(operation)) && !("*".equals(operation))
				&& !("/".equals(operation)) && !("div".equals(operation)) && !("mod".equals(operation))
				&& !("^".equals(operation)); operation = in.nextLine()) {
		...
		case "/": {
			var number3 = (float) number1;
			if (number2 == 0) {
				System.out.print("Недопустимая операция");
			} else {
				System.out.println(number1 + "/" + number2 + "=" + (number3 / number2));
			}
			break;
		}
